      ...
      <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50  text-gray-600 h-full overflow-scroll'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow-rounded'>
          ...
          <div className='flex gap-2 mt-4'>
            <p>Publish Now</p>
            <input type='checkbox' checked={isPublished} className='scale-125 cursor-pointer' onChange={e=>setIsPublished(e.target.checked)}></input>
          </div>

          <button disabled={isAdding} type="submit" className='mt-8 w-40 h-10 bg-[color:var(--color-primary)] text-white rounded cursor-pointer text-sm'>{isAdding?'Adding..':'Add Blog'}</button>
